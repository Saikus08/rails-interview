...
RSpec.describe TodoItem, type: :model do
  ...
  describe 'scopes' do
    ...
    it 'returns overdue items (due_date < now and not done)' do
      overdue = create(:todo_item, due_date: 1.day.ago, status: :in_progress)
      create(:todo_item, due_date: 1.day.ago, status: :done)
      create(:todo_item, due_date: 2.days.from_now, status: :to_do)
